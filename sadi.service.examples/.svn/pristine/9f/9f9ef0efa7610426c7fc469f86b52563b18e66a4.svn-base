...
public abstract class ServiceServletTestBase extends TestCase
{
	...
	public static final String LOCAL_URI_PREFIX = String.format("http://localhost:%s/",
	        StringUtils.defaultIfEmpty(System.getProperty("jetty-maven-plugin.port"), "8180"));
	...
	private Iterable<org.sadiframework.client.testing.TestCase> getTestCases(ServiceImpl service)
	{
	...
	public void testLocalService() throws Exception
	{
		...
		int i=0;
		int fail=0;
		for (org.sadiframework.client.testing.TestCase testCase: getTestCases(service)) {
			++i;
...
//			}
	...
	{
            String localServiceURL = getLocalServiceURL();
//	    log.debug(String.format("connecting to local service %s", localServiceURL));
	    return (ServiceImpl)ServiceFactory.createService(localServiceURL);
	}
}
